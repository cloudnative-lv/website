...
const LABELS = {
  linkedin: 'LinkedIn',
  github: 'GitHub',
  cncf: 'CNCF Kubestronaut profile',
};
...
function Icon({ kind }) {
  if (kind === 'cncf') {
    // Award/medal mark for the CNCF certification-directory profile.
    return (
      <svg
        className="w-4 h-4"
        viewBox="0 0 24 24"
  ...
  return (
    <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d={FILL_PATHS[kind]} />
    </svg>
  );
}

export default function SpeakerSocials({ info, iconClass = 'text-pink hover:text-burgundy', className = 'ml-1.5' }) {
  const links = [
