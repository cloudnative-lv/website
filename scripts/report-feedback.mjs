...
// charts and word clouds to reports/feedback/. The local op that replaces the
...
const r = await renderFeedbackReport({ feedbackBySlug, eventMeta, rostersBySlug, OUT: 'reports/feedback' });
console.log(`Wrote reports/feedback/ — ${r.responses} responses, ${r.meetups} meetups, overall ${r.overall.toFixed(2)}${r.wordclouds ? '' : ' (no word clouds)'}.`);
